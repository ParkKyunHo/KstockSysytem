...
export function TrackedStocks() {
  ...
  const [confirmDel, setConfirmDel] = useState<TrackedStockOut | null>(null);
  const perPage = 10;
  ...
  const removeTracked = useDeleteTrackedStock({
    onSuccess: () => {
      addToast({ kind: 'success', title: '추적 종료됨' });
      setConfirmDel(null);
    },
    onError: (err) => {
      addToast({
    ...
    <div>
          ...
          <table className="cds-table">
            <thead>
              <tr>
                ...
                <th>경로 분포</th>
                <th>상태</th>
                <th style={{ textAlign: 'right' }}>박스</th>
                <th style={{ textAlign: 'right' }}>포지션</th>
      ...
      {confirmDel ? (
        <Modal
          ...
          title={`${confirmDel.stock_name} 추적 종료`}
          subtitle="확인 필요"
          primary={{
            label: '추적 종료',
            onClick: () => removeTracked.mutate(confirmDel.id),
          }}
          secondary={{ label: '취소', onClick: () => setConfirmDel(null) }}
        >
          <p>
            {confirmDel.stock_name} ({confirmDel.stock_code}) 추적을 종료할까요?
          </p>
          <ul style={{ paddingLeft: 20, fontSize: 14 }}>
            <li>
              활성 박스 {confirmDel.summary.active_box_count}개 모두 취소됩니다
            </li>
            <li>시세 모니터링 중지</li>
