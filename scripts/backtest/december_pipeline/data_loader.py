# -*- coding: utf-8 -*-
"""
December Pipeline - Data Loader

past1000.csv 로딩, API 데이터 조회, parquet 캐싱 담당
"""
...
class DataLoader:
    ...
    def __init__(self, config: PipelineConfig, api_client, logger):
        self.config = config
    ...
    def load_past1000(self) -> List[Dict[str, str]]:
        """past1000.csv에서 종목 목록 로드 (ETF 제외)"""
        path = self.config.past1000_path

        if not path.exists():
            self.logger.error(f"past1000.csv 파일 없음: {path}")
            return []
        ...
        self.logger.info(f"past1000.csv 로드: {len(stocks)}개 종목 (ETF 제외)")
        return stocks
