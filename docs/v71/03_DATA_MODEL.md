...
CREATE TABLE tracked_stocks (
    ...
    -- VI 관련 플래그
    vi_recovered_today BOOLEAN NOT NULL DEFAULT FALSE,  -- 당일 VI 복구 후 재진입 금지
...
CREATE INDEX idx_tracked_stocks_path ON tracked_stocks(path_type);
...
```yaml
EXCLUDE USING gist:
  같은 종목 + 같은 경로 + 활성 상태 (EXITED 아님)인 레코드는 단 1개
  EXITED는 여러 개 허용 (이력 보존)
  
  예시:
    삼성전자 + PATH_A + TRACKING 1개  ← 가능
    삼성전자 + PATH_A + TRACKING 2개  ← 차단 (제약 위반)
    삼성전자 + PATH_A + EXITED 5개    ← 가능 (이력)
    삼성전자 + PATH_B + TRACKING 1개  ← 가능 (다른 경로)
```
