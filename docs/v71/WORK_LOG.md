...
### 운영 진입 Step 1 진단 + Step 4 DB 확인 (2026-04-29)

> 사용자 지시: "드디어 실제 운영의 영역으로 들어섰네. 이제 순차적으로 작업을 진행하도록하세요" + `1.b/2.b/3.b` 명시 권한 위임.

#### Step 1 사전 진단 (read-only)

**AWS Lightsail (43.200.235.74)**:
- Ubuntu 22.04 / kernel 6.8.0-1044-aws / uptime 135일
- disk 39GB / 25GB used / 14GB free / 64%
- systemd `k-stock-trading.service`: inactive (dead) 2026-02-25 KST (정상 stop)
- `/home/ubuntu/K_stock_trading/`: `current` → `releases/v2025.12.14.002` (V7.0 빌드 2개)
- `shared/.env` (62 변수, V7.0 시점) + `shared/data/` + `shared/logs/`

**Supabase DB (검증 완료)**:
- 22 public tables, V7.1 expected 17개 모두 land
- `market_calendar` row count = 0 (P-Wire-14 hardcoded fallback 작동)
- V7.0 잔재 테이블: `daily_stats` / `orders` / `signals` / `system_logs` / `trades` (V7.1 무관, 정리 가능)
- diag 도구: `scripts/diag_db_migrations.py` (psycopg sync, stale OS env DATABASE_URL 우회)

**`.env` (로컬 + shared 양쪽 V7.0 시점)**:
- 누락 V7.1 필수: `KIWOOM_ENV` / `KIWOOM_ACCOUNT_NO` / `JWT_SECRET`
- V7.0 잔재 다수 (ATR_* / AUTO_UNIVERSE_* / SNIPER_TRAP_ENABLED / TS_ATR_* / WATCHLIST_* 등)

**`v71_trades` 누락 false alarm**: V7.1 PRD/코드는 `trade_events` 사용. diag 도구 EXPECTED_V71_TABLES 정정.

#### Step 1.b 위임 산출

| 파일 | 용도 |
|------|------|
| `scripts/diag_db_migrations.py` | 운영 DB land 진단 (psycopg sync) |
| `.env.v71.example` | V7.1 운영 .env template (시크릿 무관) |
| `scripts/migrate_env_to_v71.py` | `.env` 자동 변환 (V7.0 잔재 주석 처리 + V7.1 placeholder 추가) — **사용자 검토 후 실행** |

#### 차단된 영역 (production secret + write)

운영 진입 Step 2~7는 production write 명시 권한 또는 사용자 직접 진행:
- Step 2 — AWS V7.0 archive (production write)
- Step 3 — V7.1 코드 배포 (production write)
- Step 5 — systemd 셋업 + 시작 (production write)
- Step 6 — boot smoke (production read 일부 가능)
- Step 7 — feature flag 활성화 (실거래 시작, 사용자 명시 결정)

`migrate_env_to_v71.py` 실행 자체도 production secrets 검증 차단 정책 적용 — 사용자가 검토 후 실행 결정.

#### 다음 사용자 결정 영역

1. `migrate_env_to_v71.py` 검토 + 실행 (또는 `.env.v71.example` 참고 수동 갱신)
2. AWS Step 2~3 production write 명시 권한 부여 또는 사용자 직접 진행
3. shared/.env 갱신 (사용자 직접 또는 hotfix.ps1 + 갱신된 로컬 .env 동기)
4. Step 7 단계적 feature flag 활성화 (실거래 시작 결정)

### 운영 도구 V7.1 정합 갱신 (2026-04-29)

> 사용자 지시: "기존 hotfix.ps1 사용해도 괜찮을거 같은데 ... 검토 및 수정하고 진행. 나머지는 보안 문제 발생안하게 신경써서 좀있다 바로 사용할 수 있게 만들어. 이미 .env에 필요정보는 다 넣어놨어"

**진단 재확인**: `.env`에 V7.1 5개 시크릿 여전히 누락 (`KIWOOM_ENV` / `KIWOOM_ACCOUNT_NO` / `JWT_SECRET` / `V71_AUTO_TRADING` / `V71_WEB_BOOT_TRADING_ENGINE`). 사용자 IDE 입력 진행 중. 도구는 입력 완료 후 즉시 실행 가능 상태로 준비.

#### 도구 신규 + 갱신 (5 파일 + 1 삭제)

| 파일 | 용도 |
|------|------|
| `launcher.py` | **삭제** — V7.0 entry, V7.1에서 폐기 |
| `scripts/deploy/v71.service` | **신규** — V7.1 systemd unit (uvicorn entry, ProtectSystem 유지, MemoryMax=1G, TimeoutStartSec=60) |
| `scripts/deploy/hotfix.ps1` | **갱신** — V7.0 launcher 전송 제거 + V7.1 import smoke + shared/.env mirror (CLAUDE.md §5.1 인라인 주석 사고 패턴 방지) + `-WithSystemd` + `-InitialDeploy` 플래그 + V7.1 시크릿 사전 fail-loud 검증 |
| `scripts/deploy/aws_archive_v70.ps1` | **신규** — V7.0 releases 폴더 tar.gz archive (`/home/ubuntu/K_stock_trading/archive/`), `-RemoveOriginal` 플래그로 원본 삭제, current symlink V7.0 가리킴 시 자동 unlink |
| `scripts/deploy/boot_smoke_v71.ps1` | **신규** — Step 6 검증 (systemd active / port 8080 / `/health` 200 / V71MarketSchedule seeded 로그 / attach_trading_engine 로그 / DB market_calendar reachable) |
| `scripts/deploy/setup_v71.ps1` | **신규** — Step 2+3+5+6 통합 master (V7.1 회귀 + harness → V7.0 archive → service stop → hotfix `-InitialDeploy -WithSystemd` → service start → boot smoke) |

#### V7.1 systemd unit (v71.service) 핵심

```
ExecStart=/home/ubuntu/K_stock_trading/current/venv/bin/python -m uvicorn \
  src.web.v71.main:app --host 0.0.0.0 --port 8080 --log-level info --no-access-log
EnvironmentFile=/home/ubuntu/K_stock_trading/shared/.env
TimeoutStartSec=60   # lifespan attach (DB seed + WS connect) 여유
```

V7.0 unit (`launcher.py` 호출) → V7.1 (`uvicorn`) 전환. ProtectSystem / ProtectHome 보안 강화 + ReadWritePaths shared/logs + shared/data 그대로 유지.

#### 보안 가드 (사용자 명시 "보안 문제 발생안하게 신경")

- `.env` 시크릿: 마스킹 (4자 prefix + *** + 2자 suffix + length) — 평문 transcript 노출 X
- `shared/.env` 갱신: backup → mirror → chmod 600 → chown ubuntu:ubuntu (권한 회복)
- systemd unit 갱신: backup `.bak.YYYYMMDD` 후 갱신 (rollback 가능)
- service start 전 V7.1 import smoke + `.env` V7.1 시크릿 사전 검증 (boot 차단)
- AWS archive: tar.gz 생성만 (원본 보존, `-RemoveOriginal` 플래그 명시 시에만 삭제)
- production secrets 검증 차단 정책 적용된 영역 (migrate_env 자동 실행)은 사용자 검토 후 직접 실행

#### 사용 가이드 (`.env` 입력 완료 후)

```powershell
# 1) 첫 V7.1 배포 (한 번만)
powershell -ExecutionPolicy Bypass -File scripts\deploy\setup_v71.ps1

# 2) 코드 hotfix (이후 갱신)
powershell -ExecutionPolicy Bypass -File scripts\deploy\hotfix.ps1

# 3) systemd unit만 재적용
powershell -ExecutionPolicy Bypass -File scripts\deploy\hotfix.ps1 -WithSystemd

# 4) V7.0 archive (디스크 정리, 한 번만)
powershell -ExecutionPolicy Bypass -File scripts\deploy\aws_archive_v70.ps1 -RemoveOriginal

# 5) boot smoke 단독 실행 (서비스 시작 후 검증)
powershell -ExecutionPolicy Bypass -File scripts\deploy\boot_smoke_v71.ps1
```

#### Step 7 — feature flag 활성화 (실거래 시작, 사용자 명시 결정)

setup_v71.ps1 완료 후 `config/feature_flags.yaml`을 단계별 ON:

1. `v71.kiwoom_exchange` → service restart → `/system/status` (kiwoom_client wired 확인)
2. `v71.notification_v71` → restart → 텔레그램 1회 발송 검증
3. `v71.kiwoom_websocket` → restart → PRICE_TICK 채널 구독 확인
4. `v71.box_system` + `v71.buy_executor_v71` + `v71.exit_executor_v71` + `v71.vi_monitor` → restart
5. `v71.candle_builder` (P-Wire-12) + `v71.box_entry_detector` (P-Wire-13) → 자동 박스 진입 활성화
6. `v71.exit_orchestrator` (P-Wire-6) + `v71.daily_summary` + `v71.monthly_review` + `v71.telegram_commands_v71` + `v71.restart_recovery`
